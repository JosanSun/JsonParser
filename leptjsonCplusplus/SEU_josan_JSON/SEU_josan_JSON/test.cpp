...
//它们的共同点都是解析成功，即返回JSON_PARSE_OK;但是解析出来的元素类型存在不同
...
//错误解析后的JSON对象的所有类型都是JSON_ERROR,但是解析错误的原因却有多种可能
